...
class UIAutomation(models.Model):
    ...
    git_url = models.CharField('git项目url', max_length=200)
    git_repo = models.CharField('git项目地址', max_length=200)
    job_name = models.CharField('job项目名称', max_length=200)
    job_url = models.CharField('job项目地址', max_length=200)
...
class MiniAutomationResult(models.Model):
    ...
    build_version = models.CharField('构建版本', max_length=100, default='')
    build_time = models.DateTimeField('构建时间', auto_now=True)
    case_num = models.IntegerField('用例数量', default=0)
    fail_num = models.IntegerField('失败用例数量', default=0)
    RESULT_CHOICES = (
        ('1', '通过'),
        ('2', '失败'),
    )
